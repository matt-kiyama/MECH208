...
def moveFans(control_value, serial, stop):
    ...
    global update_rate
    global bias_coef
    global pwm_left, pwm_right #global vars for writing to csv
...
class PIDController:
    ...
    def update(self, error, deadZoneMin, deadZoneMax):
        global proportional_term, integral_term, derivative_term #global vars for logging
        # Proportional term
...
#initialize instance of PID class
pid = PIDController(kp=Kp, ki=Ki, kd=Kd, update_rate=update_rate) 
#pid = PIDController(kp=0.028, ki=0.00001, kd=3.25, update_rate=update_rate) 
min_value = 55
base_value = 65
max_value = 75
